...
import { userIdSchema } from '@polar/types'
...
export const friendRouter = t.router({
    ...
    getReceivedRequests: protectedProcedure.query(async ({ ctx }) => {
        try {
            const requests = await friendService.getAllPendingFriendRequests(
                ctx.user.userId
            )
    ...
    getSentRequests: protectedProcedure.query(async ({ ctx }) => {
        try {
            const requests = await friendService.getAllSentRequests(
                ctx.user.userId
            )
    ...
    getAll: protectedProcedure.query(async ({ ctx }) => {
        try {
            const friends = await friendService.getAllFriendsByUserId(
                ctx.user.userId
            )
