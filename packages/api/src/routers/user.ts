...
import { profileSchema, userIdSchema, usernameSchema } from '@polar/types'
...
export const userRouter = t.router({
    getMe: protectedProcedure.query(async ({ ctx }) => {
        ...
        if (!user) return resultErr(`User not found: ${ctx.user.userId}`)
    ...
    getById: protectedProcedure.input(userIdSchema).query(async ({ input }) => {
        ...
        if (!user) return resultErr(`User not found: ${input}`)
    ...
    getFriends: protectedProcedure
        ...
        .query(async ({ input, ctx }) => {
            ...
            if (!friends)
                return resultErr(
                    `Failed to retrieve friends for user ID: ${input}`
                )

            const safeFriends = friends.friends.map((friend: any) =>
    ...
    edit: protectedProcedure
        ...
        .mutation(async ({ input, ctx }) => {
            ...
            if (!profile || !profile.profile) {
                return resultErr(
                    `Failed to modify information for user ${ctx.user.userId}`
                )
            }

            await userService.updateProfileById(profile.profile.id, input)
            return resultOk(
                `Successfully update information for ${ctx.user.userId}`
            )
        }),
})
