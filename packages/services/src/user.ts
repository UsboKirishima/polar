...
export const CACHE_KEY = {
    ...
    userAndProfileByUsername: (username: string) =>
...
export async function findUserAndProfileById(userId: string) {
    ...
    if (userProfile)
        await cacheManager.set(cacheKey, userProfile, { ttl: CACHE_TTL })
    return userProfile
}
...
export async function createUserWithProfile(
    ...
    await cacheManager.delete(
        CACHE_KEY.userAndProfileByUsername(user.profile.username)
    )
...
export async function createProfile(
    ...
    await cacheManager.delete(
        CACHE_KEY.userAndProfileByUsername(profile.username)
    )
...
export async function updateProfileById(
    ...
    if (updates.username) {
        ...
        await cacheManager.delete(
            CACHE_KEY.userAndProfileByUsername(updates.username)
        ) // New
    } else {
        await cacheManager.delete(
            CACHE_KEY.userAndProfileByUsername(profile.username)
